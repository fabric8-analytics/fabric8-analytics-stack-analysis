...
COMPONENT_PREFIX = "uranus"


def submit_evaluation_job(input_bootstrap_file, input_src_code_file, training_url, result_id):
    ...
    # we need to log into EMR cluster nodes and use application-id to view
    # YARN logs
    ...
    _logger.debug(
        "Uploading the bootstrap action to AWS S3 URI {} ...".format(s3_bootstrap_uri))
                            ...
                            "Configurations": [
                                {
                                    ...
                                    "Properties": {
                                        ...
                                        "KRONOS_SCORING_REGION": config.KRONOS_SCORING_REGION,
                                        "KRONOS_MODEL_PATH": config.KRONOS_MODEL_PATH,
            ...
            },
            {
                ...
                'HadoopJarStep': {
                    ...
                    'Args': ['spark-submit',
                             '--py-files',
                             '/home/hadoop/' + s3_key,
                             '/home/hadoop/evaluation_platform/uranus/src/' \
                             'kronos_offline_evaluation.py',
                             training_url,
                             result_id]
                }
            }
    ...
    if response.get('ResponseMetadata').get('HTTPStatusCode') == 200:
        ...
        output['evaluation_job__EMR_id'] = response.get('JobFlowId')
        output['status'] = 'work_in_progress'
        output[
            'status_description'] = "Evaluation in progress."
    else:
